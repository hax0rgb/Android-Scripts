...
class AndroidManifestAnalyzer:
    def __init__(self, manifest_path):
        ...
        except ET.ParseError as e:  # Use the fully qualified name ET.ParseError
            print(f"{Fore.RED}Could not parse {manifest_path}. Skipping.{Style.RESET_ALL}")
            print(f"{Fore.RED}ParseError: {e}{Style.RESET_ALL}")
            self.root = None
...
def scan_java_file_for_url_parameters(java_file_path):
    with open(java_file_path, 'r') as f:
        lines = f.readlines()
        for idx, line in enumerate(lines):
            # Use re.IGNORECASE for case-insensitive matching
            if re.search(r'\b(getQueryParameter|getStringExtra|getString|putExtra)\b\(["\'].*_?(url|uri).*["\']\)', line, re.IGNORECASE):
                print(f"{Fore.RED}Loading Arbitrary URL's in WebView:{Style.RESET_ALL} {Fore.MAGENTA}{java_file_path}{Style.RESET_ALL} \nline {idx + 1}: {Fore.YELLOW}{line.strip()}{Style.RESET_ALL}\n")
        ...
        if manifest_path:
            ...
            for activity_name in exported_activities:
                ...
                for java_file_path in glob(search_path, recursive=True):
                    scan_java_file_for_url_parameters(java_file_path)
