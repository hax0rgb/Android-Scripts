...
def scan_java_file_for_intent_redirection(java_file_path):
    with open(java_file_path, 'r') as f:
        lines = f.readlines()
        for idx, line in enumerate(lines):
            if re.search(r'\bgetParcelableExtra\b\(["\'].*_?intent_?.*["\']\)', line, re.IGNORECASE) or \
               re.search(r'\bget\b\(["\'].*_?intent_?.*["\']\)', line, re.IGNORECASE) or \
               re.search(r'\bgetParcelable\b\(["\'].*_?intent_?.*["\']\)', line, re.IGNORECASE):
                if "android.intent.extra.STREAM" not in line and \
                   "android.intent.extra.REFERRER" not in line and \
                   "android.intent.extra.INTENT" not in line:
                    print(f"{Fore.RED}Potential Intent Redirection found in exported activity:{Style.RESET_ALL} {Fore.MAGENTA}{java_file_path}{Style.RESET_ALL} \nline {idx + 1}: {Fore.YELLOW}{line.strip()}{Style.RESET_ALL}\n")
        ...
        if manifest_path:
            ...
            for activity_name in exported_activities:
                ...
                for java_file_path in glob(search_path, recursive=True):
                    scan_java_file_for_intent_redirection(java_file_path)
